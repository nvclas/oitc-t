	...
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
		...
		if(args.length == 2) {
			...
			if(args[0].equalsIgnoreCase("create")) {
				OfflinePlayer target = Bukkit.getOfflinePlayer(UtilityMethods.getOfflineUUID(args[1]));
			...
			if(args[0].equalsIgnoreCase("delete")) {
				...
				if(team.getFight() != null) {
					p.sendMessage(Chat.PREFIX + "§cDu kannst ein Team nicht bearbeiten w§hrend es kämpft");
					return true;
				}
				team.deleteTeam();
		...
		if(args.length >= 3) {
			...
			if(args[0].equalsIgnoreCase("remove")) {
				...
				OfflinePlayer target = Bukkit.getOfflinePlayer(UtilityMethods.getOfflineUUID(args[2]));
				if(!target.hasPlayedBefore()) {
					p.sendMessage(Chat.PREFIX + "§cDieser Spieler war noch nie online");
					return true;
				}
			...
			if(args[0].equalsIgnoreCase("setpoints")) {
				...
				if(!UtilityMethods.isNumeric(args[2]) || Integer.parseInt(args[2]) < 0) {
			...
			if(args[0].equalsIgnoreCase("create")) {
				...
				for(int i = 2; i < args.length; i++) {
					OfflinePlayer target = Bukkit.getOfflinePlayer(UtilityMethods.getOfflineUUID(args[i]));
					if(!target.hasPlayedBefore()) {
